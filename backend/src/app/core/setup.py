...
async def create_redis_cache_pool() -> None:
    redis_url = (
        getattr(settings, "redis_cache_url_resolved", None)
        or settings.REDIS_CACHE_URL
        or f"redis://{settings.REDIS_CACHE_HOST}:{settings.REDIS_CACHE_PORT}"
    )
    cache.pool = redis.ConnectionPool.from_url(redis_url)
...
async def create_redis_queue_pool() -> None:
    redis_url = (
        getattr(settings, "redis_queue_url_resolved", None)
        or settings.REDIS_QUEUE_URL
        or f"redis://{settings.REDIS_QUEUE_HOST}:{settings.REDIS_QUEUE_PORT}"
    )
    queue.pool = await create_pool(RedisSettings.from_dsn(redis_url))
...
def create_application(
    ...
    **kwargs
    ...
    environment = getattr(settings, "ENVIRONMENT", EnvironmentOption.LOCAL)
    is_production = environment == EnvironmentOption.PRODUCTION
    ...
    if isinstance(allowed_methods, str):
        ...
        methods = list(allowed_methods) if allowed_methods else ["*"]

    # BE-H2 hardening: explicit allow_headers whitelist instead of "*".
    # CORS_ALLOW_HEADERS env can extend this list (comma-separated).
    default_headers = [
        "Content-Type",
        "Authorization",
        "Accept",
    ...
    allowed_headers_env = getattr(settings, "CORS_ALLOW_HEADERS", "")
    extra_headers = [h.strip() for h in str(allowed_headers_env).split(",") if h.strip()]
    allow_headers = default_headers + extra_headers
