...
class IntentProvider(IntentProviderBase):
    ...
    def get_intent_system_prompt(self, functions_list: str) -> str:
        ...
        prompt = (
            ...
            "1. Khi user muốn TÌM KIẾM bài hát hoặc video (tìm nhạc, liệt kê, cho xem):\n"
            "   → Dùng 'search_youtube' để tìm kiếm trên YouTube\n"
            "   Ví dụ: 'tìm nhạc Sơn Tùng' → search_youtube với query='Sơn Tùng'\n"
            "   Ví dụ: 'tìm hoạt hình doremon' → search_youtube với query='hoạt hình doremon'\n\n"
            "2. Khi user muốn PHÁT NGAY (phát nhạc, mở nhạc, bật nhạc, nghe nhạc):\n"
            "   → Dùng 'self_music_play_song' - hệ thống sẽ tự phát từ thẻ nhớ hoặc tìm trên YouTube\n"
