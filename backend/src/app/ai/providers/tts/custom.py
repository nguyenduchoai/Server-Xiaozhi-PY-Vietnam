...
class TTSProvider(TTSProviderBase):
    def __init__(self, config, delete_audio_file):
        ...
        if isinstance(self.headers, str):
            try:
                self.headers = json.loads(self.headers or "{}")
            except json.JSONDecodeError:
                raise ValueError("Custom TTS config error: cannot parse headers as object")
        elif not isinstance(self.headers, dict):
            raise TypeError("Custom TTS config error: headers must be an object")

        if isinstance(self.params, str):
            try:
                self.params = json.loads(self.params)
            except json.JSONDecodeError:
                raise ValueError("Custom TTS config error: cannot parse string as object")
        elif not isinstance(self.params, dict):
            raise TypeError("Custom TTS config error: please check configuration guide")
