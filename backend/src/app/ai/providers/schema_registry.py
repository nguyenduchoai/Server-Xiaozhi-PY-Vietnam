...
TTS_VALTEC_SCHEMA = ProviderTypeSchema(
    label="Valtec Vietnamese TTS v2.1",
    description="TTS tiếng Việt nhẹ nhất (74.8M params). 5 giọng Multi-Speaker + 6 giọng Zero-Shot + Voice Clone từ audio mẫu 3-30s. Dùng custom:voice_id cho giọng đã clone.",
    fields=[
        ProviderFieldSchema(
            name="speaker",
            label="Giọng nói",
            ...
            default="NF",
            placeholder="Bấm Load để tải danh sách giọng Việt Nam",
            description="Multi-Speaker (NF/SF/NM1/SM/NM2), Built-in Clone (thu_ha, minh_duc...), hoặc Custom Clone (custom:voice_id)",
            dynamic_source="voices",
            depends_on=[],  # Local, no API key needed
        ),
        ProviderFieldSchema(
            name="speed",
            label="Tốc độ nói",
            ...
            max=2.0,
            step=0.1,
            description="1.0 = bình thường, <1.0 = nhanh hơn, >1.0 = chậm hơn",
        ),
        ProviderFieldSchema(
            name="noise_scale",
            label="Noise Scale",
            ...
            max=1.5,
            step=0.05,
            description="Độ biến thiên giọng nói. Giá trị cao = giọng biến đổi nhiều hơn.",
        ),
        ProviderFieldSchema(
            name="noise_scale_w",
            ...
            max=1.5,
            step=0.05,
            description="Độ biến thiên thời lượng âm tiết. 0.8 = mặc định.",
        ),
        ProviderFieldSchema(
            name="sdp_ratio",
            label="SDP Ratio",
            ...
            max=1.0,
            step=0.1,
            description="Stochastic Duration Predictor ratio. 0 = xác định, 1 = ngẫu nhiên.",
        ),
        ProviderFieldSchema(
            name="timeout",
            label="Timeout (giây)",
            ...
            min=10,
            max=120,
            description="Thời gian chờ tối đa. Text dài có thể cần timeout lớn hơn.",
        ),
    ],
)
